...
hmb_concat = "C:\\Users\\Pratham B\\Desktop\\UNI_Y3\\PRBX\\github\\hmb_concat\\"
#C:\Users\Pratham B\Desktop\UNI_Y3\PRBX\data\data\hmb1
hmb1 = "C:\\Users\\Pratham B\\Desktop\\UNI_Y3\\PRBX\\data\\data\\hmb1\\"
hmb2 = "C:\\Users\\Pratham B\\Desktop\\UNI_Y3\\PRBX\\data\\data\\hmb2\\"
hmb4 = "C:\\Users\\Pratham B\\Desktop\\UNI_Y3\\PRBX\\data\\data\\hmb4\\"
hmb5 = "C:\\Users\\Pratham B\\Desktop\\UNI_Y3\\PRBX\\data\\data\\hmb5\\"
hmb6 = "C:\\Users\\Pratham B\\Desktop\\UNI_Y3\\PRBX\\data\\data\\hmb6\\"
hmb1_L1 = "C:\\Users\\Pratham B\\Desktop\\UNI_Y3\\PRBX\\github\\hmb1_L1\\"
hmb1_L2 = "C:\\Users\\Pratham B\\Desktop\\UNI_Y3\\PRBX\\github\\hmb1_L2\\"
hmb1_L3 = "C:\\Users\\Pratham B\\Desktop\\UNI_Y3\\PRBX\\github\\hmb1_L3\\"

hmb = hmb1_L3
#csv = "interpolated_concat.csv"
csv = "interpolated.csv"
model = "L2_run1_L1_data" #"v3"
"""folder name       p      validation score        hmb1                                      hmb_concat                                time of day
baseline2 / base_L1 [0.4] = 0.9603071544766159  |  unnormalized RMSE:0.076193184211023210  |  unnormalized RMSE:0.26718628207974526  |  2101 / 2259
L2_run1             [0.4] = 0.9368306426943521  |  unnormalized RMSE:0.061466850156405205  |  unnormalized RMSE:0.26664577847287970  |  0230
L1_run2             [0.4] = 1.3112846602909807  |  unnormalized RMSE:0.073097509527685030  |  unnormalized RMSE:0.26851344875271980  |  1154
L1_run1             [0.4] = 0.9338469100616300  |  unnormalized RMSE:0.074320952045016540  |  unnormalized RMSE:0.26859747678298085  |  1022
baseline1           [0.0] = 1.2019231082348942  |  unnormalized RMSE:0.091637546698649420  |  unnormalized RMSE:0.31050415238103590  |  0842
L2_run2             [0.4] = 0.9316287262941125  |  unnormalized RMSE:0.071386047955908100  |  unnormalized RMSE:0.26573258051889725  |  0707
L3_run2             [0.1] = 1.0141305274657577  |  unnormalized RMSE:0.141858344489364100  |  unnormalized RMSE:0.27656263393860847  |  0533
L3_run1            [0.05] = 1.5537406764130500  |  unnormalized RMSE:0.083412931631948800  |  unnormalized RMSE:0.27032852834225100  |  0400
bare_min.py         [0.0] = N/A                 |                    0.084309519293477520  |                    0.27169170779787770  |  N/A
"""
#ANCHOR - 2
SEQ_LEN = 10
BATCH_SIZE = 4 
...
class BatchGenerator(object):
    def __init__(self, sequence, seq_len, batch_size):
        ...
        print(len(list(sequence)))
        chunk_size = 1 + (len(list(sequence)) - 1) / batch_size
        self.indices = [(i*chunk_size) % len(list(sequence)) for i in range(batch_size)]
    def next(self):
        while True:
            output = []
            for i in range(self.batch_size):
                idx = int(self.indices[i])
                left_pad = list(self.sequence)[idx - LEFT_CONTEXT:idx]
                if len(left_pad) < LEFT_CONTEXT:
                    left_pad = [list(self.sequence)[0]] * (LEFT_CONTEXT - len(left_pad)) + left_pad
                assert len(left_pad) == LEFT_CONTEXT
                leftover = len(list(self.sequence)) - idx
                if leftover >= self.seq_len:
                    result = list(self.sequence)[idx:idx + self.seq_len]
                else:
                    result = list(self.sequence)[idx:] + list(self.sequence)[:self.seq_len - leftover]
                assert len(result) == self.seq_len
                self.indices[i] = (idx + self.seq_len) % len(list(self.sequence))
...
def process_csv(filename, val=5):
    sum_f = np.float64([0.0] * OUTPUT_DIM) #np.float128([0.0] * OUTPUT_DIM)
    sum_sq_f = np.float64([0.0] * OUTPUT_DIM) #np.float128([0.0] * OUTPUT_DIM)
    lines = read_csv(filename)
...
#ANCHOR - 4
(train_seq, valid_seq), (mean, std) = process_csv(filename=csv, val=0) # concatenated interpolated.csv from rosbags 
test_seq = _read_csv(csv) # interpolated.csv for testset filled with dummy values 
train_seq = _read_csv(csv)
#valid_seq = _read_csv("interpolated_test.csv")
valid_seq = _read_csv(csv)
print("train seq = " + str(len(train_seq)))
print("valid seq = " + str(len(valid_seq)))
print("test seq = " + str(len(test_seq)))
trensq = copy.deepcopy(train_seq)
valisq = copy.deepcopy(valid_seq)
testsq = copy.deepcopy(test_seq)
#ANCHOR - 5
...
with graph.as_default():
    ...
    input_images = tf.stack([tf.io.decode_jpeg(tf.io.read_file((hmb + x))) #tf.image.decode_png(tf.read_file(x))
...
def do_epoch(session, sequences, mode):
    ...
    acc_loss = np.float64(0.0) #np.float128(0.0)
...
with tf.compat.v1.Session(graph=graph, config=tf.compat.v1.ConfigProto(gpu_options=gpu_options)) as session:
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        train_seq = copy.deepcopy(trensq)
        valid_seq = copy.deepcopy(valisq)
        test_seq = copy.deepcopy(testsq)
        """
        with open(model + "/test-predictions-epoch%d" % epoch, "w") as out:
                _, test_predictions = do_epoch(session=session, sequences=test_seq, mode="test")
                #print >> out,
                print(out)
                #print("frame_id,steering_angle")
                for img, pred in test_predictions.items():
                    img = img.replace(csv + "center\\", "")
                    out.write(img + ", " + pred)
        ...
        with open(model + "/final-predictions", "w") as out:
            result = np.float64(0.0) #np.float128(0.0)
            for img, stats in valid_predictions.items():
                #print >> out, img, stats
                #print(out)
                print(img)
                print(stats)
                out.write(str(img) + " --- " + str(stats))
                out.write("\n")
                result += stats[-1]
