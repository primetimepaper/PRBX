...
abs_path = "C:\\Users\\Pratham B\\Desktop\\UNI_Y3\\PRBX\\github\\hmb1_L3\\" #local
...
#ANCHOR - 4
(train_seq, valid_seq), (mean, std) = process_csv(filename="interpolated_train.csv", val=0) # concatenated interpolated.csv from rosbags 
test_seq = _read_csv("interpolated_test.csv") # interpolated.csv for testset filled with dummy values 
train_seq = _read_csv("interpolated_train.csv")
...
NUM_EPOCHS=40
# baseline 20 epochs complete // best epoch 4 // validation score at 4 = 0.9603071544766159, unnormalized RMSE at 4 :0.07541822348254565
# baseline + L1 20 epochs [0.4] // best epoch 1 // 0.9592403408482522
# L1 40 epochs [0.4] // 0.9368306426943521
# L3_plain validation [0.1] 30 epochs  // 1.0141305274657577 // redux = 1.55374067641305

# L2 20 epochs, [0.4] complete // best epoch 1 // validation score at 1 = 0.9940495309520355
# L2 40 epochs, [0.4] complete // best epoch ? // validation score at ? =  0.9338469100616309
# L2_redux, [0.4] 40 epochs complete // best epoch 10 // validation score at 10 = 0.9316287262941125
# L2_plain_validation, [0.5] ? epochs complete // best epoch ? // validation score at ? = 1.2019231082348942
# L1_plain validation [0.4] = 1.3112846602909807
...
with tf.compat.v1.Session(graph=graph, config=tf.compat.v1.ConfigProto(gpu_options=gpu_options)) as session:
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        print ("Last best validation score: " + str(best_validation_score))
        ...
        if valid_score < best_validation_score:
            ...
            with open("v3/test-predictions-epoch%d" % epoch, "w") as out:
                ...
                for img, pred in test_predictions.items():
                    img = img.replace(abs_path + "center\\", "")
