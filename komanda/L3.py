...
new_path = "C:\\Users\\Pratham B\\Desktop\\UNI_Y3\\PRBX\\github\\hmb1_L3\\" #local
...
# probabilities 1 - 20% chance to be augmented - equal probability for each type of augmentation - p = (0.1/6)
p = 0.1
...
def rand_L2(input_images, probs, prev_op=6, countdown=0.0, val=None, zoom=None, f=0): # augments a list of images with the same transformation (only 1 augmentation per list of images)
    output_images = []
    if (prev_op == 6) and (countdown == 0.0) and (ru(0.0, 1.0) < probs) and (len(ops.keys()) < 6):
        op = rr(6)
        while op in ops:
            op = rr(6)
        countdown = 0.1
...
zoom = None
peaks = {0}
def rand_L3(input_images, current_frame):
    lst = []
    out = copy.deepcopy(input_images)
    for op, vals in ops.items():
        # vals = [countdown, value, zoom, frame]
        # rand_L2(inputs, p, prev, c, value, zoom, frame)
        # returns   (out,    prev, c, value, zoom, frame)
        out, prev, countdown, value, zoom, frame = rand_L2(out, p, op, vals[0], vals[1], vals[2], vals[3])
        lst.append([op, prev, countdown, value, zoom, frame])
    for i in lst:
        if i[1] == 6:
            #del op from ops
            del ops[i[0]]
            ...
            #update ops
            ops[i[0]] = [i[2], i[3], i[4], i[5]]
    if (current_frame + 9) not in peaks:
        #start new op
        # rand_L2(input_images, probs, prev_op=6, countdown=0.0, val=None, zoom=None, f=0)
        out, prev, countdown, value, zoom, frame = rand_L2(out, p)
        if prev != 6:
            ops[prev] = [countdown, value, zoom, frame]
            peaks.add(current_frame + 9)
    return out
filename = Path("C:/Users/Pratham B/Desktop/UNI_Y3/PRBX/github/hmb1_L3")
while idx < len(lst):
